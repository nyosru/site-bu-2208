                ...
                <template v-for="(v, id_good) in cartAr" :key="id_good">
                  <tr v-if="v.kolvo >= 0" class="cartItem">
                    ...
                    <td>
                      ...
                      <!-- ++ {{ v.id ?? '' }} -->
                      <!-- <Br/> -->
                      <span v-if="v.id && v.id > 0 && v.id && v.id.length">
                        <router-link
                          :to="{
                            name: 'good',
                            params: { good_id: v.id },
                    ...
                    <td class="a-right m-top">
                      {{
                        v.price && v.price > 0 && v.kolvo && v.kolvo > 0
                          ? NumberFormat(v.kolvo * v.price)
                          : '-'
                      }}
                    </td>
                  </tr>
...
const stopWatch7 = watchEffect(() => {
  ...
  sumall.value = cartAr.value
    .map((item) => (item.price > 0 ? item.kolvo * item.price : 0))
    .reduce((prev, curr) => prev + curr, 0)
  // console.log(sumall);

  if (cartAr.value.find((el) => el.price == '')) {
...
const sendOrder = async (good_id) => {
  ...
  cartAr.value.forEach((e) => {
    textSms.value +=
      ...
      e.head +
      '\n' +
      (e.id && e.id > 0 && e.id && e.id.length ? '' : '(заказ) ') +
      e.id +
      ' // ' +
      (e.price != ''
        ? e.price +
